...
?>
    <tr>
      <td><?php echo anchor('invoices/view/'.$row->id, $row->invoiceNumber);?></td>
      <td><?php echo anchor('invoices/view/'.$row->id, $display_date);?></td>
      <td class="cName"><?php echo anchor('invoices/view/'.$row->id, $row->name);?></td>
      <td><?php echo anchor('invoices/view/'.$row->id, $this->settings_model->getSetting('currency_symbol') . number_format($row->subtotal, 2, '.', ''));?></td>
      <td>
	  <?php
	  
			if ($row->amount_paid >= $row->subtotal) { // paid invoices
				echo anchor('invoices/view/'.$row->id, 'closed', array('title' => 'invoice status'));
				...
				echo anchor('invoices/view/'.$row->id, 'open', array('title' => 'invoice status'));
				...
				$due_date = mysql_to_unix($row->dateIssued) + ($this->settings_model->getSetting('days_payment_due') * 60*60*24); 
				$line = "<span class='error'>" . timespan($due_date, now()) . ' overdue</span>';
